...
public class LaserIOItemRendererGas extends ItemRenderer {
    ...
    public LaserIOItemRendererGas(TextureManager textureManager, ModelManager modelManager, ItemColors itemColors, BlockEntityWithoutLevelRenderer blockEntityWithoutLevelRenderer, AbstractContainerScreen screen) {
    ...
    public void renderGuiItemDecorations(Font font, ItemStack itemstack, int x, int y, @Nullable String altText) {
        if (shouldRenderGas(itemstack, x, y, true, false)) {
            CardGasScreen cardGasScreen = (CardGasScreen) screen;
            int sloty = (int) Math.floor((y - cardGasScreen.filterStartY) / 18);
            int slotx = (int) Math.floor((x - cardGasScreen.filterStartX) / 18);
            int slot = ((5 * sloty) + slotx);
            ItemStack filter = ((CardGasScreen) screen).filter;
            ...
            if (!itemstack.isEmpty()) {
                ...
                if (!shouldRenderGas(itemstack, x, y, true, true)) {
    ...
    public boolean shouldRenderGas(ItemStack pStack, int pX, int pY, boolean includeCarried, boolean reverseBounds) {
        if (!(screen instanceof CardGasScreen)) {
            return reverseBounds;
        }
        CardGasScreen cardGasScreen = (CardGasScreen) screen;
        if (cardGasScreen.getMenu().getCarried().equals(pStack)) {
            if (includeCarried)
                return reverseBounds;
        }
        if (reverseBounds) {
            return !(MiscTools.inBounds(cardGasScreen.filterStartX, cardGasScreen.filterStartY, cardGasScreen.filterEndX - cardGasScreen.filterStartX, cardGasScreen.filterEndY - cardGasScreen.filterStartY, pX, pY));
        } else {
            if (!MiscTools.inBounds(cardGasScreen.filterStartX, cardGasScreen.filterStartY, cardGasScreen.filterEndX - cardGasScreen.filterStartX, cardGasScreen.filterEndY - cardGasScreen.filterStartY, pX, pY)) {
                return reverseBounds;
            }
        }
        ...
        TextureAtlasSprite gasStillSprite =  MekanismRenderer.getChemicalTexture(gas);        
    ...
    public void renderGuiItem(ItemStack pStack, int pX, int pY, BakedModel pBakedmodel) {
        if (!shouldRenderGas(pStack, pX, pY, true, false)) {
        ...
        RenderSystem.setShaderColor(red, green, blue, 1.0f);
        int zLevel = 100;
        float uMin = gasStillSprite.getU0();
        float uMax = gasStillSprite.getU1();
        float vMin = gasStillSprite.getV0();
        float vMax = gasStillSprite.getV1();
        Tesselator tessellator = Tesselator.getInstance();
        BufferBuilder vertexBuffer = tessellator.getBuilder();
